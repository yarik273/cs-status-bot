...
[26.08.2026 9:27] Trockenbau und Bodenlegen 75175 Pforzheim: import os
import socket
import struct
import threading
from http.server import HTTPServer, BaseHTTPRequestHandler
...
if name == "main":
    ...
    print("Telegram bot started successfully...")
    bot.polling(none_stop=True)
[02.09.2026 9:49] Trockenbau und Bodenlegen 75175 Pforzheim: import os
import socket
import struct
import threading
from http.server import HTTPServer, BaseHTTPRequestHandler
...
def get_cs_players(client, ip, port):
    ...
    try:
        ...
        return []
[02.09.2026 9:49] Trockenbau und Bodenlegen 75175 Pforzheim: def get_cs_status_full():
    """Збирає статус сервера у вигляді чистого тексту"""
    try:
        client = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        client.settimeout(2.5)
